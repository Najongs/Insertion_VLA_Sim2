#!/usr/bin/env python
"""
Convert HDF5 VLA episodes to LeRobot v3.0 dataset format.
Supports multiprocessing by splitting episodes across workers.

Dataset structure:
- HDF5 source: /data/public/NAS/Insertion_VLA_Sim2/Dataset/all_h5/
  - action: (N, 6) delta pose
  - observations/ee_pose: (N, 6)
  - observations/images/{side_camera, tool_camera, top_camera}: JPEG encoded
  - phase: (N,) - 1=Align, 2=Insert
  - timestamp: (N,)

- LeRobot target:
  - observation.state: (6,) ee_pose
  - observation.images.{side,tool,top}_camera: (480, 640, 3) video
  - action: (6,) delta pose

Usage (single process):
    python convert_hdf5_to_lerobot.py --num-workers 1

Usage (multiprocess - 8 workers):
    python convert_hdf5_to_lerobot.py --num-workers 8

Usage (run only shard 0 of 8):
    python convert_hdf5_to_lerobot.py --num-workers 8 --shard-id 0

Merge shards after all complete:
    python convert_hdf5_to_lerobot.py --merge-only --num-workers 8
...
TASK_PHASE1 = "Move the needle to the eye phantom"
TASK_PHASE2 = "Insert the needle through the eye phantom trocar opening"
CAMERA_KEYS = ["side_camera", "tool_camera", "top_camera"]
    ...
    for ep_idx, h5_path in enumerate(h5_files):
        try:
            with h5py.File(str(h5_path), "r") as h5f:
                ...
                has_phase = "phase" in h5f

                actions = h5f["action"][:].astype(np.float32)
                ee_poses = h5f["observations"]["ee_pose"][:].astype(np.float32)
                phases = h5f["phase"][:] if has_phase else None

                for frame_idx in range(num_frames):
                    state = ee_poses[frame_idx]
                    action = actions[frame_idx]

                    if has_phase and phases is not None:
                        task = TASK_PHASE2 if int(phases[frame_idx]) >= 2 else TASK_PHASE1
                    else:
                        task = TASK_PHASE1
...
def merge_shards(output_dir: Path, repo_id: str, num_shards: int):
    ...
    for i in range(num_shards):
        ...
        if shard_path.exists():
            ...
            # aggregate_datasets passes root directly to LeRobotDatasetMetadata(repo_id, root=root)
            # which sets self.root = Path(root), so root must be the shard directory itself
...
def main():
    parser = argparse.ArgumentParser(description="Convert HDF5 VLA episodes to LeRobot format")
    parser.add_argument("--hdf5-dir", type=str, default="/data/public/NAS/Insertion_VLA_Sim2/Dataset/all_h5")
    parser.add_argument("--output-dir", type=str, default="/data/public/NAS/Insertion_VLA_Sim2/TRAIN/SmolVLA/lerobot_dataset")
    parser.add_argument("--repo-id", type=str, default="insertion_vla_sim2")
    ...
    # Discover episodes
    h5_files = sorted([f for f in hdf5_dir.iterdir() if f.suffix == ".h5"])
    if args.max_episodes is not None:
        h5_files = h5_files[: args.max_episodes]
    logger.info(f"Total episodes: {len(h5_files)}")
    ...
    logger.info(f"Split {len(h5_files)} episodes into {len(shards)} shards "
                f"(~{shard_size} episodes each)")
    ...
    logger.info(f"Launching {len(shards)} worker subprocesses...")
    processes = []
